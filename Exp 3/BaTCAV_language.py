...
def load_image_tensors(class_name, sentences, random=False, root_path='Concept', image_index=None):
    if random:
        sentence = 'pick up the can'
    ...
    for filename in filenames:
        if random == False:
            sentence = random_sentence(sentences)
...
def plot_data(heatmap_data, bar_data1, bar_data2, bar_data3, std_data1, std_data2, std_data3, name, TCAV_PN=True):
    fig = plt.figure(figsize=(12, 5))
    gs = fig.add_gridspec(4, 2, width_ratios=[0.95, 0.05], height_ratios=[0.5, 0.5, 0.5, 0.5])
    ...
    fig.text(0.90, 0.58, 'Proper Language', ha='center', va='center', fontsize=20)
    fig.text(0.90, 0.40, 'Gibberish', ha='center', va='center', fontsize=20)
    fig.text(0.90, 0.20, 'Verbs', ha='center', va='center', fontsize=20)
    ...
    # with open(f'{name}_mean0.txt', 'a') as f:
    #     f.write('object : ')
    #     f.write(f'{list(bar_data1_masked)}\n')
    # with open(f'{name}_std0.txt', 'a') as f:
    #     f.write('object : ')
    ...
    # plt.savefig(f"{name}_Lift.png", format='png')
    plt.subplots_adjust(hspace=0.1)  # Remove gaps between rows
    plt.savefig(f"{name}lan_1.pdf", format='pdf', dpi=1200)
    plt.show()
...
lang2 = [
    "skdfj 12asj 5893 2467*%^ &^ 23",
    "fjdkl c33kd 3940 8175!@# $% 89",
    "qpwie b99fs 1295 3754#$@ !& 76",
    "zxcvb n66gh 5421 9836^&* *% 13",
    "plmok u55wr 7864 2319&*( ^# 42",
    "akyse l44qs 6572 0487%@# %$ 56",
    "bvgfr t22vp 3187 7695*(& #$ 34",
    "nmjqw o11lm 9538 5731!@% @! 65",
    "xswed k88ht 2409 1864%^# #! 28",
    "ecrvt f77yr 4812 6903$#@ *^ 91"
]
...
# Red
color_concept = assemble_concept_env('red', 0, load_image_tensors('Inputs/input1', sentences = lang1))
color_concept2 = assemble_concept_env('red', 1, load_image_tensors('Inputs/input2', sentences = lang1))
random_concept = assemble_concept_env('rand', 2, load_image_tensors('random_l', sentences=[], random=True))

input_tensor = load_image_tensors('Inputs/input1', sentences=[], random=True, image_index=0).to(device)
input_tensor2 = load_image_tensors('Inputs/input2',sentences=[], random=True, image_index=0).to(device)

red_tcavs = get_allscore(color_concept,random_concept,input_tensor)
red_tcavs2 = get_allscore(color_concept2,random_concept,input_tensor2)
red_tcav_data = {
...
# Gripper
grip_concept = assemble_concept_env('grip', 4, load_image_tensors('Inputs/input1', sentences = lang2))
grip_concept2 = assemble_concept_env('grip', 5, load_image_tensors('Inputs/input2', sentences = lang2))

grip_tcavs = get_allscore(grip_concept,random_concept,input_tensor)
grip_tcavs2 = get_allscore(grip_concept2,random_concept,input_tensor2)
grip_tcav_data = {
        key: [a + b for a, b in zip(grip_tcavs[key], grip_tcavs2[key])]
        for key in grip_tcavs
    }
...
# Table
table_concept = assemble_concept_env('tab', 6, load_image_tensors('Inputs/input1', sentences = lang3))
table_concept2 = assemble_concept_env('tab', 7, load_image_tensors('Inputs/input2', sentences = lang3))

table_tcavs = get_allscore(table_concept,random_concept,input_tensor)
table_tcavs2 = get_allscore(table_concept2,random_concept,input_tensor2)
table_tcav_data = {
        key: [a + b for a, b in zip(table_tcavs[key], table_tcavs2[key])]
        for key in table_tcavs
    }

for i in range(10):    
    index = i
    plt.rcParams.update({'font.size': 20})
    plot_data(action_data[index].reshape(1,72),red_tcav_data['means'][index],grip_tcav_data['means'][index],table_tcav_data['means'][index],red_tcav_data['stds'][index],grip_tcav_data['stds'][index],table_tcav_data['stds'][index], names[index])