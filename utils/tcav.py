...
def train_bcav(
    ...
    for layer in layers:
        ...
        weights_samples = torch.from_numpy(weights_samples)

        weights_samples = weights_samples[:, 1:, :] #TCAV computations don't need the intercept term
        cavs[concepts_key][layer] = CAV(
            concepts,
            layer,
            {"weights_samples": weights_samples, "classes": classes, **classifier_stats_dict},
            save_path,
            model_id,
        )
...
def train_cav(
    ...
    for layer in layers:
        ...
        assert (
            weights is not None and len(weights) > 0
        ), "Model weights cannot be None or empty"
...
class BTCAV(ConceptInterpreter):
    r"""
    This class implements ConceptInterpreter abstract class using an
    approach called Testing with Concept Activation Vectors (TCAVs),
    as described in the paper:
    ...
    def interpret(
        ...
        **kwargs: Any,
        ...
        for layer in self.layers:
            ...
            cavs_of_samples = []
            for jth_weight_sample in range(self.n_samples):
                ...
                for concepts in experimental_sets:
                    concepts_key = concepts_to_str(concepts)
                    cavs_stats = cast(Dict[str, Any], self.cavs[concepts_key][layer].stats)
                    cavs.append(cavs_stats["weights_samples"][:,:,jth_weight_sample].float().detach().tolist())
                    classes_j.append(cavs_stats["classes"])
                cavs_of_samples.append(cavs)
                classes = classes_j
            print("len of cavs_of_samples:", len(cavs_of_samples))

            scores_list = []
            for cavs in cavs_of_samples:
                # sort cavs and classes using the length of the concepts in each set
                cavs_sorted = np.array(cavs, dtype=object)[exp_set_lens_arg_sort]
                classes_sorted = np.array(classes, dtype=object)[exp_set_lens_arg_sort]
                i = 0
                while i < len(exp_set_offsets) - 1:
                    ...
                    i += 1
                scores_list.append(copy.deepcopy(scores))
